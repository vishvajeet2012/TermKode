...
export function InputBar({ onSubmit, disabled = false }: Props) {
  const { mode, toggleMode, setMode, setModel, thinking, toggleThinking } = usePromptConfig();
  ...
  const handleSubmit = useCallback(() => {
    ...
    // The command menu closes as soon as a space is typed, so `/review src/a.ts`
    // arrives here as plain text. Resolving it now is what gives custom commands
    // their arguments.
    const invocation = text.match(/^\/([A-Za-z0-9][A-Za-z0-9_-]*)\s+([\s\S]+)$/);
    const custom = invocation ? findCustomCommand(invocation[1]!) : null;

    onSubmit(custom ? renderCustomCommand(custom, invocation![2]!) : text);
    textarea.setText("");
  }, [disabled, onSubmit])
